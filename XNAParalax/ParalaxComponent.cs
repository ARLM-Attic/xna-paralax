...
namespace XNAParalax
{
    ...
    public class ParalaxBackgroundComponent : GameComponent
    {
        ...
        /// <summary>
        /// Is the Parallax Component Visible.
        /// </summary>
        ...
        public override void Draw()
        {
            if (!visible)
            {
                using (SpriteBatch sb = new SpriteBatch(this.Game.GameServices.GetService<IGraphicsDeviceService>().GraphicsDevice))
                {
                    sb.Begin(SpriteBlendMode.AlphaBlend, SpriteSortOptions.BackToFront, SpriteStateOptions.None); //There is supposed to be a bug around front to back
                    ...
                    foreach (ParalaxBackground layer in m_backgrounds)
                    {
                        ...
                        sb.Draw(texture, destCut, leftCut, Color.White, 0.0f, new Vector2(), SpriteEffects.None, layer.LayerDepth);
                        ...
                        if (layer.TileX)
                        {
                            for (int j = 1; j < tilesXCount; j++)
                            {
                                ...
                                sb.Draw(texture, v, null, Color.White, 0.0f, new Vector2(), 1.0f, SpriteEffects.None, layer.LayerDepth);
                            }
                        }
                        else if (layer.TileY)
                        {
                        ...
                        sb.Draw(texture, destRightCut, right, Color.White, 0.0f, new Vector2(), SpriteEffects.None, layer.LayerDepth);
                    }
